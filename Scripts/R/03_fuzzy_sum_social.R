...
cattle2 <- rast(here::here('data/ProcessedData/rasters/cattle_num_resc_scen.tif')) 
...
imp.fuz.sum3 <- 1 - ((wolves.1m) * (cattle2.1m) * (jurisdiction.1m))
...
writeRaster(imp.fuz.sum3, here::here('data/ProcessedData/rasters/imp_fuz_sum_cattle_scenario.tif'), overwrite=TRUE)
